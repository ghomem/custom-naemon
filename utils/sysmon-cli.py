...
def get_timeperiods():
    try:
        result = subprocess.run(
            "sudo pynag list timeperiod_name WHERE object_type=timeperiod",
            shell=True,
            check=True,
            capture_output=True,
            text=True
        ...
        for line in result.stdout.splitlines():
            line = line.strip()
            if not line or line.startswith("-") or line == "timeperiod_name":
                continue
            timeperiods.append(line.split()[0])

        return sorted(set(timeperiods))
...
def modify_service(host_name, service_name):
    ...
    while True:
        ...
        if var == "notifications_enabled":
            ...
            print("\033[92mAvailable notification states:\033[0m")
            for i, (_, label) in enumerate(notification_options, start=1):
                print(f"\033[93m{i}. {label}\033[0m")

            notification_input = input("Enter the number of the notification state to use: ").strip()
            try:
                ...
                print("\033[91mInvalid input. Please enter a number.\033[0m")
                continue
        if var == "notification_period":
            timeperiods = get_timeperiods()
            if not timeperiods:
                print("\033[91mNo timeperiods found.\033[0m")
                continue

            print("\033[92mAvailable time periods:\033[0m")
            for i, timeperiod in enumerate(timeperiods, start=1):
                print(f"\033[93m{i}. {timeperiod}\033[0m")

            period_input = input("Enter the number of the time period to use: ").strip()
            try:
